...
Your goal is to compute the sum of all their figures, all `2*81*5` of them, modulo 243 within the inclusive range `[-121;+121]`, and put it in the cell 0.
...
; - `in_f.bin` matches `in_i.bin`
; - `out_f.bin` matches `out_i.bin`
; - Cell 0 of `prg_f.bin` matches that of `prg_x.bin`
